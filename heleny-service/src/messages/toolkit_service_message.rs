...
impl Toolkit {
    pub fn new(task_id:Uuid,
    task_description: String,
    endpoint:Endpoint,
    tool_manuals:String,
    tools: HashMap<String,Box<dyn HelenyTool>>,)->Self {
    ...
    pub async fn invoke(&mut self,intent:ToolIntent)->String {
        let ToolIntent { reason, tool, command, args }=intent;
...
impl CanRequestConsent for ToolkitEndpoint {
    async fn request_consent(&self,description:String)->Result<()> {
        ...
        self.endpoint.send(USER_SERVICE, UserServiceMessage::RequestConsent { body: requestion }).await.context("发起申请失败")?;

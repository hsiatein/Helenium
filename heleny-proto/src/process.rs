...
use tokio::{io::{AsyncBufReadExt, AsyncWriteExt, BufReader, Lines},process::{self, Child, ChildStdin, ChildStdout}};
use tracing::warn;
...
impl HelenyProcessCommand {
    pub async fn spawn(&self)->Result<HelenyProcess>{
        ...
        .spawn() {
            ...
            Err(e)=>{
                Err(anyhow::anyhow!("启动子进程 {:?} 失败: {}",self,e))
            }
        }
    }
}
...
impl HelenyProcess {
    pub fn new(mut child:Child)->Result<Self>{
        ...
        let process=Self{child,stdin,stdout_lines};
        Ok(process)
    }

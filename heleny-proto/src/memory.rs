use async_openai::types::chat::{ChatCompletionRequestAssistantMessageArgs, ChatCompletionRequestMessage, ChatCompletionRequestSystemMessageArgs, ChatCompletionRequestUserMessageArgs};
...
impl DisplayMessage {
    pub fn new(id:i64,memory_entry:MemoryEntry)->Self{
        Self { id, role: memory_entry.role, time: memory_entry.time, content: memory_entry.content }
    }
}
...
impl MemoryEntry {
    pub fn new(role: ChatRole,content: MemoryContent)->Self{
        Self { role, time: Local::now(), content }
    }

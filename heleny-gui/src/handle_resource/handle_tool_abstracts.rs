...
impl FrontendHandler {
    pub async fn handle_tool_abstracts(&self,abstracts: Vec<ToolAbstract>)->Result<()>{
        self.ui_weak.upgrade_in_event_loop(|ui| {
            let mut new_abstracts:HashMap<String,ToolAbstractItem>=ui.get_tool_abstracts().iter().map(|abs|{
                (abs.name.to_string(),abs)
            }).collect();
            for abs in abstracts {
                let ToolAbstract { name, description, commands, available }=abs;
                let commands:Vec<CommandItem>=commands.into_iter().map(|(name,des)|{
                    CommandItem { description: SharedString::from(des), name: SharedString::from(name) }
                }).collect();

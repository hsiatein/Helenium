...
use heleny_proto::{FrontendMessage, memory::DisplayMessage};
use heleny_proto::memory::{ChatRole, MemoryContent};
        ...
        FrontendMessage::UpdateResource(resource)=>{
            match resource.payload {
                ...
                ResourcePayload::DisplayMessages{new,messages}=>{
                    debug!("{:?}",messages);
                    let mut messages:Vec<MessageItem>=messages.into_iter().filter_map(|msg|{
                        let DisplayMessage {id,role,time,content}=msg;
                        let content=match content {
                            MemoryContent::Text(txt)=>txt,
                            MemoryContent::Image(img)=>img,
                        };
                        Some(MessageItem{id:id as i32,is_me: role!=ChatRole::Assistant,text:content.into(),time: time.format("%Y-%m-%d %H:%M:%S").to_string().into()})
                    }).collect();
                    ui_weak.upgrade_in_event_loop(move |ui|{
                        if !new {
...
fn generate_svg_path(
    ...
    if data.is_empty() {
        return (String::new(), "0".into(), "00:00:00".into(), "00:00:00".into());
    }

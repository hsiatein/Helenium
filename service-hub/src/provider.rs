...
use heleny_proto::resource::{Resource, ResourcePayload};
...
impl Provider {
    pub fn new(name: String, resource_name: String, endpoint:Endpoint,
    receiver: watch::Receiver<ResourcePayload>,subscribers: HashSet<String>, ) -> Result<Provider> {
    ...
    pub async fn unsubscribe(&self,name:String)->Result<()>{
        self.tx.send(Command::Delete(name)).await.context("发送失败")
    }
}
...
impl ProviderWorker {
    ...
    receiver: watch::Receiver<ResourcePayload>,
    resource_name: String,subscribers: HashSet<String>,)->Self{
        Self { endpoint, receiver, subscribers, resource_name }
    }

    async fn handle(&mut self)->Result<()>{
        let resource=self.receiver.borrow_and_update().to_owned();
        for subscriber in &self.subscribers{
            if let Err(e) =self.endpoint.send(subscriber, CommonMessage::Resource(Resource { name: self.resource_name.clone(), payload: resource.clone() })).await {

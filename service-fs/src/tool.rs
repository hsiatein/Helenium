...
impl HelenyTool for FsTool {
    async fn invoke(&mut self,command:String,args:Vec<ToolArg>,_request:Box<&dyn CanRequestConsent>)->Result<String>{
        match command.as_str() {
            "ls-exchange"=>{
                let (tx,rx)=oneshot::channel();
                self.endpoint.send(FS_SERVICE, FsServiceMessage::List { dir: self.exchange_dir.clone(), feedback: tx }).await?;
                let files:Vec<PathBuf>=rx.await?.into_iter().filter_map(|file|{
                    diff_paths(file, &self.exchange_dir)
                }).collect();

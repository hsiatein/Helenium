...
impl Service for ConfigService {
    ...
    async fn handle_sub_endpoint(&mut self, msg:Box<dyn AnyMessage>) -> Result<()>{
        let work_status=msg.as_any().downcast::<WorkStatus>().map_err(|e| {anyhow::anyhow!("Downcast 消息失败: {:?}",e)})?;
        match *work_status {
            WorkStatus::WriteOver=>{
                self.post_write().await
            }
            WorkStatus::ReadOver=>{
                self.post_read().await
            }
        }
    }
...
impl ConfigService {
        ...
        match self.is_writing.take() {
            ...
            None=>{
                ...
                let handle:tokio::task::JoinHandle<Result<PathBuf>>=tokio::spawn(async move{
                    ...
                    let _=sub.send(Box::new(WorkStatus::WriteOver)).await;
                    match result {
                        Ok(_) => {
                            Ok(PathBuf::from(tmp_path))
                        }
                        Err(e) => {
                            Err(e)
                        }
    ...
    async fn post_write(&mut self)->Result<()>{
        let tmp_path=self.is_writing.take().context("未找到写入任务句柄")?.await.context("获取写入结果失败")?.context("写入配置文件失败")?;
        fs::rename(tmp_path, &self.config_path).await.context("替换配置文件失败")
    }

    async fn post_read(&mut self)->Result<()>{
        let new_value=self.is_reading.take().context("未找到读取任务句柄")?.await.context("获取读取结果失败")?.context("读取配置文件失败")?;

...
fn test_generate_key(){
    ...
    println!("{:?}",signing_key);
    let pem=signing_key.to_pkcs8_pem(LineEnding::LF).expect("转pem失败").to_string();
    ...
    verify.verify_strict(&challenge, &signature).expect("验证失败");
    let _doc=signing_key.to_pkcs8_der().expect("生成失败").write_der_file("./key.der");
    // println!("{:?}",doc);
}
...
fn test_read_key(){
    ...
    let pri_key_str=std::fs::read_to_string(pri_key_path).expect("没有设置HELENIUM_KEY").trim().replace("\r\n", "\n");
    ...
    println!("key: {:?}",key);
    println!("{:?}",pri_key_str);
    let mut signing_key: SigningKey = SigningKey::from_pkcs8_pem(&pri_key_str).expect("SigningKey转换失败");
    let pub_key: VerifyingKey = VerifyingKey::from_public_key_pem(&pub_key_str).expect("VerifyingKey转换失败");
    let challenge=generate_challenge();
    let signature=signing_key.sign(&challenge);
    pub_key.verify_strict(&challenge, &signature).expect("验证失败");
}
...
fn test_read_key_loopback() {
    ...
    // 3. 立即反序列化 (不要经过任何手动复制)
    let recovered_key = ed25519_dalek::SigningKey::from_pkcs8_pem(&pem)
        .expect("闭环解析失败");
...
fn test_key_file_io_loopback() {
    ...
    let pri_key_path = std::env::var("HELENIUM_KEY").unwrap_or_else(|_| "test_ed25519.key".to_string());

    // 2. 生成标准的 SigningKey
    let original_key = ed25519_dalek::SigningKey::generate(&mut rand::rngs::OsRng);
    println!("{}",original_key.verifying_key().to_public_key_pem(LineEnding::LF).unwrap());
    // 3. 序列化为 PEM

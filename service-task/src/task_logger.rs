...
use heleny_proto::{ResourcePayload, TaskLog, TaskAbstract, TaskStatus};
...
impl TaskLogger {
    pub fn new(watch_sender: watch::Sender<ResourcePayload>,)->Self {
        Self { task_logs: HashMap::new(), subscriber: HashMap::new(), watch_sender, running: true }
    }
    ...
    pub fn get_abstracts(&self)->Vec<TaskAbstract>{
        self.task_logs.iter().map(|(id,log)|{
            TaskAbstract {
                id:*id,
...
impl TaskLoggerHandle {
    pub fn new(log_tx: mpsc::Sender<TaskLoggerMessage>,
    handle_tx:  mpsc::Sender<TaskLoggerCommand>,
    handle:tokio::task::JoinHandle<()>,)->Self {
        Self { log_tx, handle_tx, handle: Some(handle) }
    }
    ...
    pub async fn get_log(&self,id:Uuid)->Result<TaskLog>{
        ...
        self.handle_tx.send(TaskLoggerCommand::GetLog { id, feedback: tx }).await?;
        let log=rx.await?;
        Ok(log)
    }
    ...
    pub async fn set_status(&self,id:Uuid,status:TaskStatus)->Result<()> {
        self.handle_tx.send(TaskLoggerCommand::SetStatus { id, status }).await?;
        Ok(())
    }

    pub async fn subscribe(&self,id:Uuid,sender:mpsc::Sender<TaskLog>)->Result<()> {
        self.handle_tx.send(TaskLoggerCommand::Subscribe { id, sender }).await?;
        Ok(())
    }
...
pub async fn launch_task_logger()->(TaskLoggerHandle,watch::Receiver<ResourcePayload>) {
    ...
    let (watch_tx, watch_rx)=watch::channel::<ResourcePayload>(ResourcePayload::TaskAbstract { task_abstracts: Vec::new() });
